...
if __name__ == '__main__':
    ...
    if not spellcheck:
        ...
        for text in text_list[0]:
            b=TextBlob(text)
            text_start.append(b.correct())
        
    if len(text_start)>0:
        stopwords=word_cloud_history(text_start)
        #capitalised word cloud
        caps_only=1
        cleaned_sentences,frequencies,proper_nouns,tokens,cleaned=text_process(text_start,stopwords)
        #sorted by speech parts
        
        all_cleaned=list_flat(cleaned)
        
        all_cleaned=extra_clean(all_cleaned)
        
        nouns,adj,adv,propernouns=nltk_functions.parts_extract(all_cleaned)
        
    
        
        stopwords=word_cloud_history(adj)
        stopwords=word_cloud_history(nouns)
        stopwords=word_cloud_history(adv)
        for record in proper_nouns:
            if not record[2]=="NNP" or record[2]=="NNPS":
                propernouns.append(record[0])
        stopwords=word_cloud_history(propernouns)
