...
# -*- coding: utf-8 -*-
"""
A script to scrape tweets according to a selected query, select tweets with multiple users mentioned and to friend those users.
Also has an option to blacklist certain users and not friend them
...
#The script searches twitter for tweets containing (i) up to 2 hashtags you pick and (ii) lists of usernames prefixed with @
#When it finds them, it compiles a list of users. Then it attempts to follow a random subset.

#Here you put in your hashtags - currently the code allows two. Leave the second one blank if you don't want it.
query_1="#SocialistSunday"
query_2="#SocialistAnyDay"

#I prefer not to add people with a lot of followers. This variable sets the upper ceiling for followers that you want people you follow to have.
max_follower_count=10000

#not sure what this does
max_tweets=100000

#Option to load a list from file and skip the search. You need to have run the script once with load_file=0 to create the file with a list.
load_file=0

#Whether to shuffle the list before following - advise to not change
random_shuffle=1

#If you want to only build a list and not actually follow anyone, set follow=False
follow=True
lookup=False
if follow:
    lookup=True
...
query1=query_1+" -filter:retweets"
query2=query_2+" -filter:retweets"
TP="tweepy"


#These variables try to sift out the rubbish - tweets you don't actually want
t=8#min no. of people listed in tweet. You want lists of @'s, not tweets that just randomly contain the hashtag.
u=5#max no. of hashtags allowed in tweet. I hate multihashtaggers. 
trigger_length=400#character length beyond which we don't just accept every @ but try to omit the mentions at the start. The problem is that 'ats' mentioned in tweet body look the same to 'ats' mentioned in the 'reply-to'. The latter are far less interesting.
#This is a hamfisted way to remove them. 

#Here you put in bad boys whom you don't want to follow - racists, antisemites, Russian trolls...
exclude_list={}

try:
    wainwright_list=array_load(save_dir+"problem_users.csv")#problem_users.csv is a file in directory save_dir that should contain the users' names in a column. If it doesn't exist, this will still work.
except UnicodeDecodeError:
    print("problem list not found")
    wainwright_list=[]


for entry in wainwright_list:
    
    exclude_list['@'+entry[0].lower()]="problem list"

#As well as importing problem users from file, you can write them in the code explicitely
#Some arbitrary examples, just to show this works: you can view the object exclude_list as a dictionary
...
ignore_posts_from={}#Some users are just plain annoying and add in random 'ats'. Here, you can ignore their tweets altogether.
...
def users_unfollow(friends_names,exclude_list):
    ...
    for former_friend in final:
        ...
        a=0
        b=100
        timegap=random.uniform(a,b)
        time.sleep(50+timegap)
        if isinstance(exclude_list,dict):
